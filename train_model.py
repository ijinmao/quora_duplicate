...
def train_model(model_name):
    ...
    for train_index, val_index in kf_gen:
        ...
        # Build model
        model = build_model(model_name, embedding_matrix, word_index, char_index)
...
def build_model(model_name, embedding_matrix, word_index, char_index):
    if model_name == 'basic_rnn':
        return build_basic_rnn(embedding_matrix, word_index, char_index)
    elif model_name == 'distance':
        return build_distance(embedding_matrix, word_index, char_index)
    elif model_name == 'bimpm':
        return build_bimpm(embedding_matrix, word_index, char_index)
    elif model_name == 'decomposable_attention':
        return build_attention(embedding_matrix, word_index, char_index)
    elif model_name == 'compare_aggregate':
        return build_compare_aggre(embedding_matrix, word_index, char_index)
...
def get_model_config(name):
    if name == 'basic_rnn':
        ...
        config = ResDistanceConfig
    elif name == 'decomposable_attention':
        config = AttentionConfig
    elif name == 'compare_aggregate':
        config = CompareAggreConfig
    return config


def main():
    model_name = 'basic_rnn'
    models, test_x1, test_x2, test_ids = train_model(model_name)
    test_model(model_name, models, test_x1, test_x2, test_ids)
